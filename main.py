...
def main():
    ...
    logger.debug("Verifying that the files 'fullchain.pem' and 'privkey.pem' exist")
    # Verifying that SSL certificate exist on the server
    check_files = dict(map(lambda x: (x, True) if x in local_cert_list else (x, False), cer_files.values()))
    ...
    if check_download:
        ...
        [download(url=f"{url}/{filename}", dest_path=path) for filename in cer_files.values()]
        return
    ...
    try:
        ...
        current_cert = x509.load_pem_x509_certificate(open(f"{path}/{cer_files['cert']}", "rb").read(), default_backend())
    ...
    try:
        ...
        if not r.ok:
            logger.error({"status_code": r.status_code, "message": r.text})
            return
        new_cert = x509.load_pem_x509_certificate(r.text.encode(), default_backend())
...
if __name__ == "__main__":
    logging.basicConfig(
        handlers=[RotatingFileHandler('./log/certificate_updater.log', maxBytes=100000, backupCount=10)],
        level=logging.DEBUG,
